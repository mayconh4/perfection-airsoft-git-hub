...
async function getOrders() {
    // Pegando a URL do projeto primeiro (caso tenha mudado ou para garantir)
    const infoRes = await fetch(`https://api.supabase.com/v1/projects/${PROJECT_REF}`, {
        headers: { 'Authorization': `Bearer ${TOKEN}` }
    });
    const info = await infoRes.json();
    const projectUrl = `https://${PROJECT_REF}.supabase.co`;

    // Pegando a SERVICE ROLE KEY
