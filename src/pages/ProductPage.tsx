        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-3 mb-3">
            ...
            <button
              ...
              className={`flex-1 border font-black py-3 px-4 flex items-center justify-center gap-2 transition-all uppercase tracking-widest text-[10px] focus:outline-none ${isInWishlist(product.id) ? 'border-red-500 text-red-500 hover:bg-red-500/10' : 'border-primary/40 hover:border-primary text-primary hover:bg-primary/5'}`}
            >
              <span className="material-symbols-outlined text-base" style={isInWishlist(product.id) ? {fontVariationSettings: "'FILL' 1"} : {}}>favorite</span>
              {isInWishlist(product.id) ? 'Favoritado' : 'Favoritos'}
            </button>
          </div>

          {/* Linha inferior: Adicionar ao Kit (grande, destaque total) */}
          <button
            onClick={async () => { await addItem(product.id); }}
            disabled={product.stock === 0}
            aria-label={product.stock === 0 ? 'Produto esgotado' : `Adicionar ${product.name} ao carrinho`}
            className="w-full bg-primary hover:bg-amber-300 text-background-dark font-black py-5 px-8 flex items-center justify-center gap-3 transition-all active:scale-[0.98] uppercase tracking-widest text-sm disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background-dark mb-8"
          >
