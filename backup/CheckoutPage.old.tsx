...
export function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (isPaymentActive && !showSuccessModal) {
      ...
      // 2. POLLING DE ELITE (Consulta direta ao Supabase no banco de dados)
      // Substituindo o proxy que falha em produção por uma conexão direta e resiliente
      interval = setInterval(async () => {
        try {
          if (!orderId) return;

          console.log('[Checkout] Ciclo de consulta direta (DB)...');
          ...
          if (data?.status === 'pago' || data?.status === 'confirmed') {
            console.log('[Checkout] Status confirmado via DB Polling!');
          ...
          console.warn('[Checkout] Alerta no polling (DB):', err.message);
        }
      }, 5000); // Polling a cada 5 segundos para não sobrecarregar
    }
